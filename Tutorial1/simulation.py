...
def simulation(env_range, pos, robot_rad, sens_range, dT, weights, graphics=False):
...
def evolutionaryAlgorithm(num_iter, population_size, ndim, rn_range, benchmark_function, offspring):
    ...
    for _ in range(num_iter):
        ...
        for i in range(population_size):
            sim_start = time()
            fitness = benchmark_function(env_range, pos, robot_rad, sens_range, dT, weights=population[i],
                                         graphics=False)
        ...
        for i in range(population_size):
            population[i] = np.reshape(
                [np.mean([population[i][j][k], population[np.random.randint(population.shape[0])][j][k]]) for j in
                 range(population.shape[1]) for k in range(population.shape[2])], (2, 14))
        # Output
        average_genotype = np.mean(population[0:], axis=0)
        print('Average :', average_genotype)
        fitness = benchmark_function(env_range, pos, robot_rad, sens_range, dT, weights=average_genotype,
                                     graphics=False)
        fitnesess[_] = fitness_all
        averages[_] = fitness_all.mean()
        bests[_] = fitness_all.max()
        # plt.clear()
        plt.plot(range(_), averages[0:_], color="red")
        plt.plot(bests[0:_], color="green")
        # ax2.boxplot(fitness_all)
        plt.pause(0.0001)
